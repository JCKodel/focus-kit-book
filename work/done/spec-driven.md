...
* Context, amended after the author's review: before the excerpts, section 3 describes the starting project (the `Appointment` type, the in-memory list, `book(clientName, startsAt)`, no cancel, no slot rule, no tests) and shows `brief.md` lines 5 to 15 (Feature, Answers, Principles), in English in the English edition and translated in the Portuguese one, which says the original is in English.
* Excerpts, amended after the author's review: the same unit of the decision in each tool (a cancellation in time is accepted, exactly 24 hours before included), copied byte for byte from the run folder, in English in both editions; the Portuguese edition follows each with its full translation; each edition then says in one sentence what it shows:
  * Spec Kit: `speckit/feature/specs/001-cancel-appointment/spec.md`, lines 13 to 32 (User Story 1 with its scenarios).
  * OpenSpec: `openspec/feature/openspec/changes/add-client-cancellation/specs/appointment-cancellation/spec.md`, lines 9 to 18 (the first requirement with its two scenarios).
  * focus-kit: `focus-kit/feature/work/cancel.md`, lines 9 to 32 (Objective and Behaviour).
  None of those lines holds an em dash; /apply checked before copying, since the em dash exemption covers only the run folder.
  The page first named three single scenarios (Spec Kit lines 30 to 32, OpenSpec 16 to 18, focus-kit line 16); the author found them meaningless without context, the focus-kit line above all.
...
* Author's review, after the first /apply: the excerpts were English only in the Portuguese edition, with one sentence and no translation; neither edition said what project or problem they were about; the focus-kit excerpt was one line that leaned on the line above it. Fixed by the two amendments of the Contract above: the project and the brief before the excerpts, whole units, a full translation in Portuguese. The page also says why focus-kit writes "staff" where the brief says "client" (its `/analyze` chose clinic staff as the only user, `focus-kit/questions.md`).
* docs/04 §Writing the book gains two rules from that review: **Context before an excerpt** (the reader understands every artifact from the previous chapters alone; the project and problem come first; an excerpt is a whole unit, the same unit across compared tools) and **Teach, do not only show**; §Evidence now asks the Portuguese edition for the full translation of an English excerpt.
* Author's second review: in the PDF every wrapped code line hung 5em to the right. The cause is pandoc's own print style for highlighted code (`pre > code.sourceCode > span { text-indent: -5em; padding-left: 5em; }`), which the book's first long code lines exposed. `pandoc/pdf.css` and `pandoc/epub.css` now reset it, so a wrapped line starts at the margin; docs/01 says so. The Spec Kit lines, hard-wrapped by the tool at about 100 columns, still break once more on A5 (about 85 columns at 8pt); they stay byte for byte.
* Proof: `make verify` green. The three excerpts, taken from each edition's fenced blocks, compared as lines with the file lines the page names: identical in both editions; none holds an em dash. `make book` builds; weasyprint's two `user-select` warnings are the known ones of chapter 2. The chapter brings the book's first table, so docs/05 §5 asked for screens: `work/done/spec-driven-<en|pt>-site-<light|dark>.png` (headless Chrome, 1280 by 4200, over the built site) and `work/done/spec-driven-<en|pt>-pdf-brief.png`, `-pdf-table.png` and `-pdf-excerpts-<n>.png` (`pdftoppm`; after the reviews, the brief on page 13 and the table on page 14 of both PDFs, excerpts on pages 15 to 17 in English and 16 to 19 in Portuguese). The table fits the A5 page. Seen and left: the Spec Kit lines run to about 100 characters and OpenSpec writes each requirement as one long line, so the site scrolls them sideways and the PDF wraps them with a hanging indent; rewrapping would break byte for byte. The Portuguese translations are the book's own text, wrapped at 78 columns. The site screens are 1280 by 6400 after the review.
